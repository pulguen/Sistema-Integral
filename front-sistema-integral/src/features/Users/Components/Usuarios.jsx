...
export default function Usuarios() {
  ...
  const [localUsuarios, setLocalUsuarios] = useState([]); // Lista local para sincronización

  // Actualiza la lista local cada vez que cambia la lista de usuarios del contexto
  useEffect(() => {
  ...
  const handleDeleteUser = useCallback(async (id) => {
    try {
      ...
      if (result.isConfirmed) {
        ...
        await fetchUsuarios(); // Actualiza los datos
  ...
  const handleEditUser = useCallback(async (updatedUser) => {
    try {
  ...
  }, [editUsuario, fetchUsuarios]);

  const handleAddUser = useCallback(async (newUser) => {
    try {
      await addUsuario(newUser); // Ya actualiza el contexto
      setShowAddModal(false); // Cerrar modal al agregar usuario
    } catch (error) {
  ...
  const {
    ...
    { columns: columnsUsuarios, data: filteredUsuarios, initialState: { pageIndex: 0, pageSize: 10 } },
  ...
  return (
    <div className="table-responsive mt-2 usuarios-section">
      ...
      <CustomButton onClick={() => setShowAddModal(true)} className="mb-3" aria-label="Agregar Usuario">
              ...
              <tr {...headerGroup.getHeaderGroupProps()} key={headerGroup.id}>
                {headerGroup.headers.map((column) => (
                  ...
                  >
                    ...
                    {column.canSort ? (
                      column.isSorted ? (
                        column.isSortedDesc ? <FaSortDown className="ms-2" /> : <FaSortUp className="ms-2" />
      ...
      <NewUserModal show={showAddModal} handleClose={() => setShowAddModal(false)} handleSubmit={handleAddUser} />
